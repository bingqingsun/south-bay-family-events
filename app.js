...
const copy = {
  zh: {
    ...
    saved: '已收藏', results: count => `发现 ${count} 个活动`, savedResults: count => `已收藏 ${count} 个活动`, emptyFiltered: '当前筛选条件下暂无活动。试试放宽日期、年龄或类别。', emptyAll: '暂时没有已核验的活动，请稍后再试。', updateUnavailable: '最近更新信息暂不可用 · 来自官方活动来源', update: date => `最近更新：${date}（南湾时间）· 官方来源`, ageFact: label => `适合：${label}`, costFact: label => `费用：${label}`, ageUnknown: '年龄未注明', costUnknown: '费用未注明', viewDetails: source => source ? `查看 ${source} 详情` : '查看活动详情', directions: '导航', expandDescription: '展开简介', collapseDescription: '收起简介', showSessions: count => `查看 ${count} 个场次`, hideSessions: '收起场次', save: title => `收藏：${title}`, unsave: title => `取消收藏：${title}`, showAll: '显示全部活动', showSaved: '只查看收藏活动', timeUnavailable: '请点击活动详情查看活动时间'
  },
  en: {
    ...
    saved: 'Saved', results: count => `${count} activities found`, savedResults: count => `${count} saved activities`, emptyFiltered: 'No activities match these filters. Try widening the date, age, or category.', emptyAll: 'No verified activities are available right now. Please try again soon.', updateUnavailable: 'Latest refresh information is unavailable · Official sources', update: date => `Last updated: ${date} · Official sources`, ageFact: label => `Ages: ${label}`, costFact: label => `Cost: ${label}`, ageUnknown: 'Age not specified', costUnknown: 'Cost not specified', viewDetails: source => source ? `View ${source} details` : 'View activity details', directions: 'Directions', expandDescription: 'Show description', collapseDescription: 'Hide description', showSessions: count => `Show ${count} sessions`, hideSessions: 'Hide sessions', save: title => `Save: ${title}`, unsave: title => `Remove saved activity: ${title}`, showAll: 'Show all activities', showSaved: 'Show saved activities', timeUnavailable: 'See organizer details for the event time'
  }
};
...
function render() {
  ...
  visible.forEach(event => {
    const session = activeSession(event); const sessions = matchingSessions(event); const node = template.content.cloneNode(true); const image = event.image || `assets/fallback/${event.type || 'community'}.png`; const imageArea = node.querySelector('.card-image');
    ...
    const sessionToggle = node.querySelector('.sessions-toggle'); const sessionList = node.querySelector('.sessions-list'); const allSessions = eventSessions(event); sessionToggle.hidden = allSessions.length < 2; sessionToggle.dataset.eventId = event.id; sessionToggle.setAttribute('aria-expanded', 'false'); sessionToggle.textContent = t('showSessions')(allSessions.length); sessionList.id = `sessions-${event.id}`; sessionToggle.setAttribute('aria-controls', sessionList.id); allSessions.forEach(item => { const row = document.createElement('li'); const sessionLink = document.createElement('a'); sessionLink.href = item.url || event.url; sessionLink.target = '_blank'; sessionLink.rel = 'noopener'; sessionLink.textContent = dateLabel(item.dateValue) || item.date; row.append(sessionLink); sessionList.append(row); });
    const link = node.querySelector('.source-link'); link.href = session.url || event.url; link.firstChild.textContent = `${t('viewDetails')(event.source)} `;
...
grid.addEventListener('click', e => { const sessionToggle = e.target.closest('.sessions-toggle'); if (sessionToggle) { const list = document.querySelector(`#sessions-${sessionToggle.dataset.eventId}`); const isExpanded = !list.hidden; list.hidden = isExpanded; sessionToggle.textContent = isExpanded ? t('showSessions')(list.children.length) : t('hideSessions'); sessionToggle.setAttribute('aria-expanded', String(!isExpanded)); return; } const toggle = e.target.closest('.description-toggle'); if (toggle) { const description = document.querySelector(`#description-${toggle.dataset.eventId}`); const isExpanded = description.classList.toggle('is-expanded'); toggle.textContent = isExpanded ? t('collapseDescription') : t('expandDescription'); toggle.setAttribute('aria-expanded', String(isExpanded)); return; } const button = e.target.closest('.heart'); if (!button) return; const id = button.dataset.id; const legacyIds = JSON.parse(button.dataset.legacyIds || '[]'); const saved = state.saved.includes(id) || legacyIds.some(legacyId => state.saved.includes(legacyId)); state.saved = saved ? state.saved.filter(item => item !== id && !legacyIds.includes(item)) : [...state.saved.filter(item => !legacyIds.includes(item)), id]; localStorage.setItem('southBaySaved', JSON.stringify(state.saved)); render(); });
