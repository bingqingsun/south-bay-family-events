...
function sortEvents(items) {
  ...
  if (state.sort === 'recommended' && window.SouthBayRecommendation) return window.SouthBayRecommendation.rankRecommendedEvents(items, { todayKey: localToday(), getDateValue: event => activeSession(event).dateValue });
...
function render() {
  ...
  visible.forEach((event, eventIndex) => {
    ...
    const link = node.querySelector('.source-link'); link.href = session.url || event.url; link.firstChild.textContent = `${t('viewDetails')} `; const rank = eventIndex + 1; link.addEventListener('click', () => { const parameters = { event_id: event.id, rank, sort_type: state.sort, recommendation_score: event.recommendationScore || 0, editor_pick: event.editorPick === true, activity_category: event.type || 'other', organizer: event.source || 'unknown' }; track('event_card_clicked', parameters); track('activity_details_opened', parameters); });
